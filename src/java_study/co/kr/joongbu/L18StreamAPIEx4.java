...
public class L18StreamAPIEx4 {
	...
	public static void main(String[] args) {
		...
		// any, all, none, Match(): 검사(중간연산 filter 유사함)

		// collect(): 수집 (주간연산 map과 유사)

		int[] scoreArrCKM = { 88, 50, 70, 100, 30, 90 };
		int[] scoreArrHKD = { 90, 100, 99, 100, 85, 92 };

		// 성적관리 프로그램
		// 장학금 대상자 2명이 있는데 모든 과목의 성적이 80이상이어야 장학금이 나온다. (검사식을 구현하세요)

		long c = Arrays.stream(scoreArrCKM).filter((s) -> s < 80).count();
		System.out.println("최경민씨의 성직이 80점 이하인 과목의 수: " + c);
		...
		System.out.println("최경민씨의 성적이 60 미만인 것이 1개라도 있나요? " + test);
